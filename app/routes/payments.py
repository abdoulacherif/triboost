    ...
    try:
        ...
        if not profile_data:
            ...
            try:
                ...
                profile_data = insert.data[0] if insert.data else {"full_name": "", "is_activated": False}
            except Exception:
        ...
        try:
            ...
            print(f"[PAYMENT] Erreur insert activation: {e}")
...
async def create_withdrawal(request: Request):
    try:
        ...
        if amount < 2000:
            raise HTTPException(status_code=400, detail="Minimum 2 000 FCFA")
...
async def my_withdrawals(request: Request):
    try:
        ...
        admin = get_supabase_admin()
        result = admin.table("withdrawals").select("*").eq("user_id", user_response.user.id).order("created_at", desc=True).execute()
...
async def create_recharge(request: Request):
    try:
        ...
        if amount < 1000:
            raise HTTPException(status_code=400, detail="Minimum 1 000 FCFA")
        if not country or not operator or not phone or not full_name:
            raise HTTPException(status_code=400, detail="Tous les champs sont obligatoires")
        ...
        try:
            ...
            print(f"[PAYMENT] Erreur create_recharge: {e}")
...
async def my_recharges(request: Request):
    try:
        ...
        admin = get_supabase_admin()
        result = admin.table("recharges").select("*").eq("user_id", user_response.user.id).order("created_at", desc=True).execute()
...
async def leekpay_webhook(request: Request):
    try:
        ...
        if status == "paid":
            ...
            if user_id:
                try:
                    is_activation = str(metadata.get("reference", "")).startswith("TRIBOOST-")
                    if is_activation:
                        admin.rpc("activate_account", {
                            "p_user_id": user_id,
                            "p_payment_method": "leekpay",
                        ...
                        admin.rpc("confirm_recharge", {"p_checkout_id": checkout_id}).execute()
                    print(f"[WEBHOOK] OK pour {user_id}")
                except Exception as e:
