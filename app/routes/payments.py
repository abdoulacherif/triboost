...
<script>
  ...
  async function submitWithdraw(e) {
    ...
    try {
      ...
      loadProfile();
      loadWithdrawals();
      setTimeout(() => document.querySelector('[data-tab="history"]').click(), 800);
    } catch (err) {
  ...
  async function loadWithdrawals() {
    ...
    try {
      ...
      list.innerHTML = items.map(w => {
        ...
        return '<div class="history-item"><div class="hi-icon ' + icon + '">💸</div><div class="hi-info"><div class="hi-title">' + escapeHtml(w.operator) + ' · ' + escapeHtml(w.phone) + '</div><div class="hi-meta"><span>' + formatDate(w.created_at) + '</span><span class="hi-badge ' + w.status + '">' + statusLabel + '</span></div></div><div class="hi-amount negative">-' + Number(w.amount).toLocaleString('fr-FR') + ' F</div></div>';
      }).join('');
    } catch (e) {
  ...
  async function loadRecharges() {
    ...
    try {
      ...
      list.innerHTML = items.map(r => {
        ...
        return '<div class="history-item"><div class="hi-icon recharge">💰</div><div class="hi-info"><div class="hi-title">' + escapeHtml(r.operator) + ' · ' + escapeHtml(r.phone) + '</div><div class="hi-meta"><span>' + formatDate(r.created_at) + '</span><span class="hi-badge ' + r.status + '">' + statusLabel + '</span></div></div><div class="hi-amount positive">+' + Number(r.amount).toLocaleString('fr-FR') + ' F</div></div>';
      }).join('');
    } catch (e) {
      list.innerHTML = '<div class="empty-state"><h3>Erreur</h3></div>';
    }
