...
async def get_lesson_full(lesson_id: str, request: Request):
    try:
        ...
        # Exercices
        exercises = admin.table("lesson_exercises").select(
            "id, title, instructions, exercise_type, options, points, sort_order"
        ).eq("lesson_id", lesson_id).order("sort_order").execute()
...
async def finish_session(lesson_id: str, request: Request):
    try:
        ...
        try:
            unlock = admin.table("path_unlocks").select("id").eq(
                "user_id", user_id
            ).execute()
...
async def external_get_lesson(lesson_id: str, x_api_key: str = Header(None)):
    try:
        ...
        admin = get_supabase_admin()
        key = admin.table("platform_api_keys").select("*").eq(
            "api_key", x_api_key
        ...
        lesson = admin.table("course_lessons").select(
            "id, title, content, video_url, pdf_url, objectives, "
            "duration_minutes, day_number, gain_amount, loss_amount"
        ).eq("id", lesson_id).execute()

        if not lesson.data:
            raise HTTPException(status_code=404, detail="Leçon introuvable")

        blocks = admin.table("lesson_content_blocks").select(
            "*"
        ).eq("lesson_id", lesson_id).order("sort_order").execute()
...
async def create_api_key(request: Request):
    try:
        ...
        check = admin.table("profiles").select(
            "is_admin"
...
async def list_api_keys(request: Request):
    try:
        ...
        keys = admin.table("platform_api_keys").select(
            "*"
