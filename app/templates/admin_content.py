...
<style>
  ...
  .item.banned { opacity: 0.5; border-left: 4px solid #d32f2f; }
...
<div class="app">
  <header class="topbar">
    ...
    <div class="page-title">Modération <span class="admin-badge">ADMIN</span></div>
...
<div class="modal-overlay" id="userModal">
  <div class="modal-content">
    <div class="modal-title" id="userModalTitle">Modifier l'utilisateur</div>
...
<!-- MODAL FORMATION (créer/modifier) -->
<div class="modal-overlay" id="formationModal">
  <div class="modal-content">
    ...
    <div class="form-group">
      <label>Prix (FCFA) — mettre 0 si gratuit</label>
...
<script>
  ...
  async function loadUsers() {
    ...
    try {
      ...
      list.innerHTML = items.slice(0, 100).map(u => {
        return '<div class="item ' + (u.is_banned ? 'banned' : '') + '">' +
          '<div class="item-header">' +
          '<div style="flex:1;min-width:0;">' +
          '<div class="item-title">' + escapeHtml(u.full_name || 'Sans nom') + (u.is_admin ? ' <span class="badge-admin" style="padding:1px 6px;font-size:9px;border-radius:4px;">ADMIN</span>' : '') + (u.is_banned ? ' <span class="badge-banned" style="padding:1px 6px;font-size:9px;border-radius:4px;">BANNI</span>' : '') + '</div>' +
          ...
          '<button class="btn-sm ' + (u.is_banned ? 'btn-approve' : 'btn-danger') + '" onclick=\\'toggleBan("' + u.id + '", ' + (u.is_banned ? 'true' : 'false') + ')\\'>' + (u.is_banned ? '✓ Débannir' : '🚫 Bannir') + '</button>' +
          '</div></div>';
      }).join('');
  ...
  async function saveUser() {
    ...
    try {
      const res = await fetch('/api/admin/content/user/' + editingUserId + '/update', {
  ...
  function toggleBan(uid, isBanned) {
    if (isBanned) {
      fetch('/api/admin/content/user/' + uid + '/update', {
  ...
  async function loadMarket() {
    ...
    try {
      const res = await fetch('/api/admin/content/market', { headers: headers() });
      const data = await res.json();
      ...
      list.innerHTML = items.map(i => {
        return '<div class="item ' + (i.is_banned ? 'banned' : '') + '">' +
          ...
          '<div class="item-sub">📍 ' + escapeHtml(i.city || '-') + ' · 📅 ' + new Date(i.created_at).toLocaleDateString('fr-FR') + '</div>' +
          '</div>' +
  ...
  async function deleteMarket(id) {
    ...
    try {
      await fetch('/api/admin/content/market/' + id, { method: 'DELETE', headers: headers() });
      loadMarket();
  ...
  async function loadServices() {
    ...
    try {
      const res = await fetch('/api/admin/content/services', { headers: headers() });
      const data = await res.json();
      const items = data.items || [];
      ...
      list.innerHTML = items.map(i => {
        return '<div class="item ' + (i.is_banned ? 'banned' : '') + '">' +
          ...
          '<div class="item-sub">📅 ' + new Date(i.created_at).toLocaleDateString('fr-FR') + '</div>' +
          '</div>' +
          '<span class="item-badge ' + (i.is_banned ? 'badge-banned' : (i.is_active ? 'badge-active' : 'badge-inactive')) + '">' + (i.is_banned ? 'Banni' : (i.is_active ? 'Actif' : 'Inactif')) + '</span>' +
          '</div>' +
          '<div class="item-actions">' +
  ...
  async function deleteService(id) {
    ...
    try {
      await fetch('/api/admin/content/services/' + id, { method: 'DELETE', headers: headers() });
      loadServices();
    } catch (e) { alert('⚠ Erreur'); }
  }
  ...
  async function loadFranchises() {
    ...
    try {
      const res = await fetch('/api/admin/content/franchises', { headers: headers() });
      const data = await res.json();
      const items = data.items || [];
      ...
      list.innerHTML = items.map(i => {
        return '<div class="item">' +
          ...
          '<div class="item-sub">👤 ' + escapeHtml(i.user_name || '') + '</div>' +
          '<div class="item-sub">💰 Payé: ' + Number(i.price_paid || 0).toLocaleString('fr-FR') + ' F</div>' +
          '<div class="item-sub">📊 Gains: ' + Number(i.total_earned || 0).toLocaleString('fr-FR') + ' F</div>' +
          '<div class="item-sub">📅 ' + new Date(i.created_at).toLocaleDateString('fr-FR') + '</div>' +
          '</div>' +
  ...
  async function toggleFranchise(id) {
    try {
      await fetch('/api/admin/content/franchises/' + id + '/toggle', { method: 'POST', headers: headers() });
      loadFranchises();
    } catch (e) { alert('⚠ Erreur'); }
  }
  ...
  async function deleteFranchise(id) {
    ...
    try {
      await fetch('/api/admin/content/franchises/' + id, { method: 'DELETE', headers: headers() });
      loadFranchises();
    } catch (e) { alert('⚠ Erreur'); }
  }
  ...
  async function loadFormations() {
    ...
    try {
      const res = await fetch('/api/admin/content/formations', { headers: headers() });
      const data = await res.json();
      const items = data.items || [];
      if (items.length === 0) { list.innerHTML = '<div class="empty">Aucune formation</div>'; return; }
      ...
      list.innerHTML = items.map(i => {
        return '<div class="item ' + (i.is_banned ? 'banned' : '') + '">' +
          ...
          '<div class="item-sub">💰 ' + (i.is_free ? 'GRATUIT' : Number(i.price || 0).toLocaleString('fr-FR') + ' F') + '</div>' +
          '<div class="item-sub">👁 ' + (i.views || 0) + ' vues · 📅 ' + new Date(i.created_at).toLocaleDateString('fr-FR') + '</div>' +
          '</div>' +
          ...
          '<button class="btn-sm ' + (i.is_published ? 'btn-warn' : 'btn-approve') + '" onclick=\\'toggleFormationPublish("' + i.id + '", ' + (i.is_published ? 'true' : 'false') + ')\\'>' + (i.is_published ? '⏸ Dépublier' : '▶ Publier') + '</button>' +
  ...
  async function toggleFormationPublish(id, isPublished) {
    try {
      await fetch('/api/admin/content/formations/' + id + '/update', {
        method: 'POST', headers: headers(),
  ...
  async function deleteFormation(id) {
    ...
    try {
      await fetch('/api/admin/content/formations/' + id, { method: 'DELETE', headers: headers() });
      loadFormations();
    } catch (e) { alert('⚠ Erreur'); }
  }
  ...
  async function loadPaths() {
    ...
    try {
      const res = await fetch('/api/admin/content/paths', { headers: headers() });
      const data = await res.json();
      const items = data.items || [];
      ...
      list.innerHTML = items.map(i => {
        return '<div class="item">' +
          ...
          '</div>' +
          '<div class="item-actions">' +
          '<button class="btn-sm btn-warn" onclick=\\'editPath("' + i.id + '")\\'>✏️ Modifier</button>' +
  ...
  async function editPath(id) {
    try {
      ...
      await fetch('/api/admin/content/paths/' + id + '/update', {
        method: 'POST', headers: headers(),
        body: JSON.stringify({
          title: newTitle,
  ...
  async function deletePath(id) {
    if (!confirm('Supprimer ce parcours ? Les formations liées seront retirées du parcours.')) return;
    try {
      await fetch('/api/admin/content/paths/' + id, { method: 'DELETE', headers: headers() });
      loadPaths();
    } catch (e) { alert('⚠ Erreur'); }
  }
  ...
  async function confirmBan() {
    ...
    try {
      if (currentBanAction.type === 'user') {
        await fetch('/api/admin/content/user/' + currentBanAction.id + '/update', {
          method: 'POST', headers: headers(),
          body: JSON.stringify({ is_banned: true, admin_note: reason })
        });
        loadUsers();
      } else if (currentBanAction.type === 'market') {
        await fetch('/api/admin/content/market/' + currentBanAction.id + '/ban', {
          method: 'POST', headers: headers(), body: JSON.stringify({ reason: reason })
        });
        loadMarket();
      } else if (currentBanAction.type === 'service') {
        await fetch('/api/admin/content/services/' + currentBanAction.id + '/ban', {
          method: 'POST', headers: headers(), body: JSON.stringify({ reason: reason })
        });
        loadServices();
      }
  ...
  document.querySelectorAll('.tab').forEach(tab => {
    tab.addEventListener('click', function() {
      ...
      if (this.dataset.tab === 'services') loadServices();
      if (this.dataset.tab === 'franchises') loadFranchises();
      if (this.dataset.tab === 'formations') loadFormations();
      if (this.dataset.tab === 'paths') loadPaths();
    });
  });
