...
<style>
  ...
  @media (min-width: 768px) {
    ...
    .stats-row { grid-template-columns: repeat(4, 1fr); padding: 0 16px; }
  }
</style>
</head>
<body>
...
<div class="app" id="app">
  ...
  <div id="content" style="display:none;">
    ...
    <div class="progress-card" id="progressCard" style="display:none;">
      <div class="progress-label"><span>Progression</span><span id="progressText">0 / 0</span></div>
      <div class="progress-bar-bg"><div class="progress-fill" id="progressFill" style="width: 0%"></div></div>
      <div class="rewards-row">
        <div class="reward-chip"><div class="v" id="statGained">+0 F</div><div class="l">Gagné</div></div>
        <div class="reward-chip red"><div class="v" id="statLost">-0 F</div><div class="l">Perdu</div></div>
      </div>
    </div>

    <div class="unlock-card" id="unlockCard" style="display:none;">
      <div style="font-size: 40px;">🔒</div>
      <div style="font-size: 13px; color: #6d4c00; font-weight: 700; margin-top: 8px;">Débloquez ce parcours pour</div>
      <div class="unlock-price" id="unlockPrice">- <span>F</span></div>
...
<script>
  ...
  function render() {
    ...
    document.getElementById('heroIcon').textContent = p.icon || '📚';
    ...
    if (p.is_unlocked) {
      ...
      document.getElementById('progressText').textContent = completed + ' / ' + totalLessons;
      setTimeout(function() { document.getElementById('progressFill').style.width = pct + '%'; }, 100);
      document.getElementById('statGained').textContent = '+' + fmt(p.total_gained) + ' F';
      document.getElementById('statLost').textContent = '-' + fmt(p.total_lost) + ' F';
    } else {
      document.getElementById('unlockCard').style.display = 'block';
      document.getElementById('unlockPrice').innerHTML = fmt(p.unlock_price || 5000) + ' <span>FCFA</span>';
      document.getElementById('unlockSub').textContent = totalLessons + ' leçons · ' + (p.final_bonus || 500) + ' F de bonus final';
    }

    const container = document.getElementById('chaptersContainer');
    if (!p.chapters || p.chapters.length === 0) {
      container.innerHTML = '<div class="empty-state"><div class="icon">📚</div><h3>Aucune leçon</h3></div>';
      return;
    }

    container.innerHTML = p.chapters.map((ch, idx) => {
      const lessons = ch.lessons || [];
      const completed = lessons.filter(l => (p.completed_lesson_ids || []).indexOf(l.id) !== -1).length;
      return '<div class="chapter" id="chapter-' + ch.id + '">' +
        ...
        '</div>' +
        '<div class="chapter-lessons">' + lessons.map(l => renderLesson(l, p)).join('') + '</div>' +
        '</div>';
    }).join('');
  }
  ...
  function renderLesson(l, p) {
    ...
    let statusClass = 'locked', statusIcon = '🔒', btnHTML = '<button class="lesson-btn" disabled>Bloqué</button>';
    if (isDone) {
      statusClass = 'done'; statusIcon = '✓';
      btnHTML = '<button class="lesson-btn" disabled>Terminé</button>';
    } else if (isToday) {
      statusClass = 'today'; statusIcon = '▶';
  ...
  async function unlockPath() {
    ...
    try {
      const res = await fetch('/api/courses/paths/' + pathId + '/unlock', { method: 'POST', headers: headers() });
      const data = await res.json();
  ...
  function renderQuiz(questions) {
    ...
    box.innerHTML =
      '<div class="quiz-head"><div class="quiz-title">' + escapeHtml(currentLesson.title) + '</div>' +
      '<button class="quiz-close" onclick="closeQuiz()">✕</button></div>' +
      (currentLesson.content ? '<div class="quiz-content">' + escapeHtml(currentLesson.content) + '</div>' : '') +
      (currentLesson.content_url ? '<a href="' + currentLesson.content_url + '" target="_blank" style="display:block;padding:14px;background:#e8f5e9;border-radius:12px;text-decoration:none;color:#2e7d32;font-weight:700;font-size:13px;margin-bottom:16px;">📥 Voir le contenu</a>' : '') +
      '<div style="font-size:12px;color:#757575;margin-bottom:12px;">📅 Jour ' + currentLesson.day_number + ' · ✅ +' + fmt(currentLesson.gain_amount) + ' F · ❌ -' + fmt(currentLesson.loss_amount) + ' F</div>' +
      '<div id="questionsList">' + questions.map((q, i) => renderQuestion(q, i)).join('') + '</div>' +
  ...
  function renderQuestion(q, index) {
    ...
    return '<div class="q-item">' +
      ...
      opts.map((opt, i) =>
        ...
        '<div class="circle"></div><div>' + escapeHtml(opt) + '</div></div>'
      ).join('') + '</div>';
  }

  function selectAnswer(qid, idx, el) {
    currentAnswers[qid] = idx;
  ...
  async function submitQuiz() {
    ...
    if (Object.keys(currentAnswers).length < totalQ) {
  ...
  function showResult(r) {
    const isWin = r.net >= 0;
    const bonusHTML = r.path_completed ? '<div class="result-bonus">🎉 Parcours terminé ! +' + fmt(r.bonus) + ' F</div>' : '';
    document.getElementById('quizBox').innerHTML =
      ...
      '</div>' +
      '<div class="result-net ' + (isWin ? 'up' : 'down') + '">' + (r.net >= 0 ? '+' : '') + fmt(r.net) + ' F</div>' +
      bonusHTML +
