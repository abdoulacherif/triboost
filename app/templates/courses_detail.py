...
<style>
  ...
  @keyframes pulse-gold {
    0%, 100% { box-shadow: 0 0 0 0 rgba(251,192,45,0.5); }
  ...
  .q-item { background: #fff; border: 1.5px solid #e0e0e0; border-radius: 14px; padding: 16px; margin-bottom: 12px; transition: border-color 0.2s; }
  .q-item:has(.q-opt.selected) { border-color: var(--green); background: #f1f8e9; }
  ...
  .result-icon { font-size: 64px; margin-bottom: 12px; animation: popIn 0.5s cubic-bezier(0.16, 1, 0.3, 1); }
  ...
  .result-bonus { background: linear-gradient(135deg, #fff8e1, #fffde7); border: 2px solid var(--gold); border-radius: 14px; padding: 14px; margin: 16px 0; font-size: 13px; color: #6d4c00; font-weight: 700; animation: bounce 0.6s ease; }
  ...
  @media (min-width: 768px) {
    ...
    .chapter { margin: 0 16px 16px; }
    .chapter-header { padding: 20px; }
    .chapter-title { font-size: 17px; }
    .lesson-item { padding: 16px 20px; }
  }
</style>
</head>
<body>
    ...
    <div class="progress-card" id="progressCard" style="display:none;">
      ...
      <div class="rewards-row">
        <div class="reward-chip">
          <div class="v" id="statGained">+0 F</div>
          <div class="l">Gagné</div>
        </div>
        <div class="reward-chip red">
          <div class="v" id="statLost">-0 F</div>
...
<script>
  ...
  function render() {
    ...
    document.getElementById('statsRow').innerHTML =
      ...
      '<div class="stat-box"><div class="v">' + (p.total_days || 30) + '</div><div class="l">Jours</div></div>' +
    ...
    if (p.is_unlocked) {
      ...
      document.getElementById('unlockSub').textContent =
        totalLessons + ' leçons · ' + (p.final_bonus || 500) + ' F de bonus final';
    }

    // Chapitres
    const container = document.getElementById('chaptersContainer');
    if (!p.chapters || p.chapters.length === 0) {
      container.innerHTML = '<div class="empty-state"><div class="icon">📚</div><h3>Aucune leçon</h3><p>Le parcours sera bientôt disponible.</p></div>';
      return;
    }
    ...
    container.innerHTML = p.chapters.map((ch, idx) => {
      ...
      return '<div class="chapter" id="chapter-' + ch.id + '">' +
        ...
        '<div class="chapter-meta">' + completed + ' / ' + lessons.length + ' leçons · ' + escapeHtml(ch.description || '') + '</div>' +
        '</div>' +
        '<div class="chapter-arrow">▾</div>' +
        '</div>' +
        '<div class="chapter-lessons">' +
        lessons.map(l => renderLesson(l, p)).join('') +
        '</div></div>';
    }).join('');
  }
  ...
  function renderLesson(l, p) {
    ...
    const isLocked = !isUnlocked || (!isDone && !isToday);
  ...
  async function unlockPath() {
    ...
    try {
      ...
      setTimeout(function() { loadPath(); }, 800);
    } catch (e) {
  ...
  function renderQuestion(q, index) {
    ...
    return '<div class="q-item" id="q-' + q.id + '">' +
  ...
  function selectAnswer(questionId, optionIndex, el) {
    ...
    const parent = el.closest('.q-item');
